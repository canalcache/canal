...
CH_LDB1:                                          ; preds = %CH_LDB
  ...
  %6 = srem i32 %3, %5
  %7 = icmp eq i32 %6, 0
  %8 = load i32* @i, align 64
  call void @__CSIM_Load(i64 24588, i64 3)
  br i1 %7, label %CH_LDB3, label %CH_LDB4

CH_LDB3:                                          ; preds = %CH_LDB1
  %9 = sext i32 %8 to i64
  %10 = getelementptr inbounds [200 x i32]* @a, i32 0, i64 %9
  store i32 42, i32* %10, align 4
  %CH_OFFSET = mul i64 4, %9
...
CH_LDB4:                                          ; preds = %CH_LDB1
  %11 = load i32* %0, align 4
  %12 = mul nsw i32 %8, %11
  %13 = sext i32 %12 to i64
  %14 = getelementptr inbounds [200 x i32]* @a, i32 0, i64 %13
  store i32 42, i32* %14, align 4
  %CH_OFFSET66 = mul i64 4, %13
  %CH_LEFT67 = add i64 %CH_OFFSET66, 6293888
  %CH_COND68 = icmp ult i64 %CH_LEFT67, 6293952
  br i1 %CH_COND68, label %CH_TRUE_BLOCK73, label %CH_ELSE_BLOCK69

CH_LDB6:                                          ; preds = %CH_TRUE_BLOCK129, %CH_TRUE_BLOCK128, %CH_ELSE_BLOCK124, %CH_TRUE_BLOCK123, %CH_TRUE_BLOCK118, %CH_TRUE_BLOCK113, %CH_TRUE_BLOCK108, %CH_TRUE_BLOCK103, %CH_TRUE_BLOCK98, %CH_TRUE_BLOCK93, %CH_TR
  %15 = load i32* @i, align 64
  call void @__CSIM_Load(i64 24588, i64 3)
  %16 = add nsw i32 %15, 1
  store i32 %16, i32* @i, align 64
  call void @__CSIM_Store(i64 24588, i64 3)
  br label %CH_LDB

; <label>:17                                      ; preds = %CH_LDB
  ret void

CH_ELSE_BLOCK:                                    ; preds = %CH_LDB3
  %CH_OFFSET7 = mul i64 4, %9
  %CH_LEFT8 = add i64 %CH_OFFSET7, 6293888
  %CH_COND9 = icmp ult i64 %CH_LEFT8, 6294016
  br i1 %CH_COND9, label %CH_TRUE_BLOCK14, label %CH_ELSE_BLOCK10

CH_TRUE_BLOCK:                                    ; preds = %CH_LDB3
  call void @__CSIM_Store(i64 24585, i64 2)
  br label %CH_LDB6

CH_ELSE_BLOCK10:                                  ; preds = %CH_ELSE_BLOCK
  %CH_OFFSET11 = mul i64 4, %9
  %CH_LEFT12 = add i64 %CH_OFFSET11, 6293888
  %CH_COND13 = icmp ult i64 %CH_LEFT12, 6294080
  br i1 %CH_COND13, label %CH_TRUE_BLOCK19, label %CH_ELSE_BLOCK15

CH_TRUE_BLOCK14:                                  ; preds = %CH_ELSE_BLOCK
  call void @__CSIM_Store(i64 24585, i64 3)
  br label %CH_LDB6

CH_ELSE_BLOCK15:                                  ; preds = %CH_ELSE_BLOCK10
  %CH_OFFSET16 = mul i64 4, %9
  %CH_LEFT17 = add i64 %CH_OFFSET16, 6293888
  %CH_COND18 = icmp ult i64 %CH_LEFT17, 6294144
  br i1 %CH_COND18, label %CH_TRUE_BLOCK24, label %CH_ELSE_BLOCK20

CH_TRUE_BLOCK19:                                  ; preds = %CH_ELSE_BLOCK10
  call void @__CSIM_Store(i64 24586, i64 0)
  br label %CH_LDB6

CH_ELSE_BLOCK20:                                  ; preds = %CH_ELSE_BLOCK15
  %CH_OFFSET21 = mul i64 4, %9
  %CH_LEFT22 = add i64 %CH_OFFSET21, 6293888
...
CH_ELSE_BLOCK25:                                  ; preds = %CH_ELSE_BLOCK20
  %CH_OFFSET26 = mul i64 4, %9
  %CH_LEFT27 = add i64 %CH_OFFSET26, 6293888
...
CH_ELSE_BLOCK30:                                  ; preds = %CH_ELSE_BLOCK25
  %CH_OFFSET31 = mul i64 4, %9
  %CH_LEFT32 = add i64 %CH_OFFSET31, 6293888
...
CH_ELSE_BLOCK35:                                  ; preds = %CH_ELSE_BLOCK30
  %CH_OFFSET36 = mul i64 4, %9
  %CH_LEFT37 = add i64 %CH_OFFSET36, 6293888
...
CH_ELSE_BLOCK40:                                  ; preds = %CH_ELSE_BLOCK35
  %CH_OFFSET41 = mul i64 4, %9
  %CH_LEFT42 = add i64 %CH_OFFSET41, 6293888
...
CH_ELSE_BLOCK45:                                  ; preds = %CH_ELSE_BLOCK40
  %CH_OFFSET46 = mul i64 4, %9
  %CH_LEFT47 = add i64 %CH_OFFSET46, 6293888
...
CH_ELSE_BLOCK50:                                  ; preds = %CH_ELSE_BLOCK45
  %CH_OFFSET51 = mul i64 4, %9
  %CH_LEFT52 = add i64 %CH_OFFSET51, 6293888
...
CH_ELSE_BLOCK55:                                  ; preds = %CH_ELSE_BLOCK50
  %CH_OFFSET56 = mul i64 4, %9
  %CH_LEFT57 = add i64 %CH_OFFSET56, 6293888
...
CH_ELSE_BLOCK60:                                  ; preds = %CH_ELSE_BLOCK55
  %CH_OFFSET61 = mul i64 4, %9
  %CH_LEFT62 = add i64 %CH_OFFSET61, 6293888
  %CH_COND63 = icmp ult i64 %CH_LEFT62, 6294688
  br i1 %CH_COND63, label %CH_TRUE_BLOCK65, label %CH_LDB6

CH_TRUE_BLOCK64:                                  ; preds = %CH_ELSE_BLOCK55
  call void @__CSIM_Store(i64 24588, i64 1)
  br label %CH_LDB6

CH_TRUE_BLOCK65:                                  ; preds = %CH_ELSE_BLOCK60
  call void @__CSIM_Store(i64 24588, i64 2)
  br label %CH_LDB6

CH_ELSE_BLOCK69:                                  ; preds = %CH_LDB4
  %CH_OFFSET70 = mul i64 4, %13
  %CH_LEFT71 = add i64 %CH_OFFSET70, 6293888
  %CH_COND72 = icmp ult i64 %CH_LEFT71, 6294016
  br i1 %CH_COND72, label %CH_TRUE_BLOCK78, label %CH_ELSE_BLOCK74

CH_TRUE_BLOCK73:                                  ; preds = %CH_LDB4
  call void @__CSIM_Store(i64 24585, i64 2)
  br label %CH_LDB6

CH_ELSE_BLOCK74:                                  ; preds = %CH_ELSE_BLOCK69
  %CH_OFFSET75 = mul i64 4, %13
  %CH_LEFT76 = add i64 %CH_OFFSET75, 6293888
  %CH_COND77 = icmp ult i64 %CH_LEFT76, 6294080
  br i1 %CH_COND77, label %CH_TRUE_BLOCK83, label %CH_ELSE_BLOCK79

CH_TRUE_BLOCK78:                                  ; preds = %CH_ELSE_BLOCK69
  call void @__CSIM_Store(i64 24585, i64 3)
  br label %CH_LDB6

CH_ELSE_BLOCK79:                                  ; preds = %CH_ELSE_BLOCK74
  %CH_OFFSET80 = mul i64 4, %13
  %CH_LEFT81 = add i64 %CH_OFFSET80, 6293888
...
CH_ELSE_BLOCK84:                                  ; preds = %CH_ELSE_BLOCK79
  %CH_OFFSET85 = mul i64 4, %13
  %CH_LEFT86 = add i64 %CH_OFFSET85, 6293888
...
CH_ELSE_BLOCK89:                                  ; preds = %CH_ELSE_BLOCK84
  %CH_OFFSET90 = mul i64 4, %13
  %CH_LEFT91 = add i64 %CH_OFFSET90, 6293888
...
CH_ELSE_BLOCK94:                                  ; preds = %CH_ELSE_BLOCK89
  %CH_OFFSET95 = mul i64 4, %13
  %CH_LEFT96 = add i64 %CH_OFFSET95, 6293888
...
CH_ELSE_BLOCK99:                                  ; preds = %CH_ELSE_BLOCK94
  %CH_OFFSET100 = mul i64 4, %13
  %CH_LEFT101 = add i64 %CH_OFFSET100, 6293888
...
CH_ELSE_BLOCK104:                                 ; preds = %CH_ELSE_BLOCK99
  %CH_OFFSET105 = mul i64 4, %13
  %CH_LEFT106 = add i64 %CH_OFFSET105, 6293888
...
CH_ELSE_BLOCK109:                                 ; preds = %CH_ELSE_BLOCK104
  %CH_OFFSET110 = mul i64 4, %13
  %CH_LEFT111 = add i64 %CH_OFFSET110, 6293888
...
CH_ELSE_BLOCK114:                                 ; preds = %CH_ELSE_BLOCK109
  %CH_OFFSET115 = mul i64 4, %13
  %CH_LEFT116 = add i64 %CH_OFFSET115, 6293888
...
CH_ELSE_BLOCK119:                                 ; preds = %CH_ELSE_BLOCK114
  %CH_OFFSET120 = mul i64 4, %13
  %CH_LEFT121 = add i64 %CH_OFFSET120, 6293888
...
CH_ELSE_BLOCK124:                                 ; preds = %CH_ELSE_BLOCK119
  %CH_OFFSET125 = mul i64 4, %13
  %CH_LEFT126 = add i64 %CH_OFFSET125, 6293888
  %CH_COND127 = icmp ult i64 %CH_LEFT126, 6294688
  br i1 %CH_COND127, label %CH_TRUE_BLOCK129, label %CH_LDB6

CH_TRUE_BLOCK128:                                 ; preds = %CH_ELSE_BLOCK119
  call void @__CSIM_Store(i64 24588, i64 1)
  br label %CH_LDB6

CH_TRUE_BLOCK129:                                 ; preds = %CH_ELSE_BLOCK124
  call void @__CSIM_Store(i64 24588, i64 2)
  br label %CH_LDB6
}
...
; <label>:46                                      ; preds = %0
  call void @__assert_fail(i8* getelementptr inbounds ([19 x i8]* @.str14, i32 0, i32 0), i8* getelementptr inbounds ([6 x i8]* @.str15, i32 0, i32 0), i32 59, i8* getelementptr inbounds ([11 x i8]* @__PRETTY_FUNCTION__.main, i32 0, i32 0)) #5
  unreachable
...
; <label>:51                                      ; preds = %47
  call void @__assert_fail(i8* getelementptr inbounds ([25 x i8]* @.str16, i32 0, i32 0), i8* getelementptr inbounds ([6 x i8]* @.str15, i32 0, i32 0), i32 60, i8* getelementptr inbounds ([11 x i8]* @__PRETTY_FUNCTION__.main, i32 0, i32 0)) #5
  unreachable
